  ...
  current.text(country && country.name || '')
    ...
    xhr.addEventListener("readystatechange", function () {
        if (this.readyState === this.DONE) {
            ...
            const objj = JSON.parse(this.responseText)
    ...
    google.charts.load("current", {packages:["corechart"]});
         ...
         function drawChart() {
           var data = google.visualization.arrayToDataTable(objj);
...
function printValues(objj) {
    document.getElementById("myTable").innerHTML = ""
    for(var k in objj) {
        if(objj[k] instanceof Object) {
            printValues(objj[k]);
        } else {
            //des.innerHTML(objj[k] + "<br>");
            //document.getElementById("des").innerHTML += k + " - " + objj[k] + "<br>"
            var table = document.getElementById('myTable');
                        var newRow = table.insertRow(-1);
                        var newCell1 = newRow.insertCell(0);
                        var newCell2 = newRow.insertCell(1);

                        newCell1.innerHTML = k;
                        newCell2.innerHTML = objj[k];
